...
<div class="my-12 flex flex-wrap justify-center gap-16 px-6 md:px-10 mb-7">
<?php foreach ($cards as $card): ?>
    <div class="card glow relative w-[260px] mb-4 cursor-pointer rounded-2xl border border-gray-300 bg-white p-6 text-center shadow-md transition-all" data-title="<?= $card['title'] ?>" data-desc="<?= $card['desc'] ?>">
        <?php if (!empty($card['badge'])): ?>
        <span class="absolute top-2 left-2 bg-gradient-to-r from-[#EC8623] to-[#D24753] text-white text-xs px-3 py-1 rounded-full font-semibold"><?= $card['badge'] ?></span>
        <?php endif; ?>
        <img src="<?= $card['image'] ?>" alt="<?= $card['title'] ?>" class="mx-auto h-[140px] object-contain">
        <p class="mt-2 text-lg font-semibold text-gray-700"><?= $card['title'] ?></p>        
    </div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<?php endforeach; ?>
</div>
<div id="modal">
    <div class="modal-content">
        ...
        <a href="contact.php" class="btn-appointment pulse">Book Appointment</a>
    </div>
</div>
<script>
